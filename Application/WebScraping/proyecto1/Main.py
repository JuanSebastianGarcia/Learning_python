...
class Proyecto1():
    ...
    def extraer_contenedor_principal(self,soup):
        """
    ...
    def unificarDatos(self,df):
        """
            Se extren los datos ya existentes y se unifican con los nuevos
        """
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        direccion = os.path.join(base_dir,'proyecto1/data','noticias.csv')
    ...
    def cargarDatos(self,soup):
        ...
        noticias = self.extraer_contenedor_principal(soup)
    ...
    def main(self):
        ...
        contenido_html = self.obtener_contenido_pagina(URL)
        if contenido_html:

            # Crear el objeto BeautifulSoup con el contenido HTML
            soup = BeautifulSoup(contenido_html, 'html.parser')
            
            self.cargarDatos(soup)
